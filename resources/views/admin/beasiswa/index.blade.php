                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card shadow-sm rounded-4 border-0 h-100">
                        <div class="card-body d-flex flex-column">
                            ...
                            {{-- Kuota --}}
                            <p class="mb-2"><span class="badge bg-warning text-dark">Kuota:
                                    {{ $beasiswa->kuota ?? 'N/A' }}</span></p>

                            {{-- Deskripsi --}}
                            <p class="text-muted small">{{ Str::limit($beasiswa->deskripsi, 150) }}</p>

                            {{-- Tanggal --}}
                            <p class="mb-1"><strong>Mulai:</strong>
                                {{ \Carbon\Carbon::parse($beasiswa->tanggal_mulai)->translatedFormat('d F Y') }}</p>
                            <p class="mb-2"><strong>Berakhir:</strong>
                                ...
                                @auth
                                    @if (auth()->user()->role === 'admin')
                                        ...
                                        <form action="{{ route('beasiswa.destroy', $beasiswa->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="btn btn-outline-danger btn-sm rounded-pill px-3">Hapus</button>
                                        </form>
                                    @endif
                                @endauth
